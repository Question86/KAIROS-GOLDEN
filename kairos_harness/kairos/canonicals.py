...
def refresh_dynamic_canonicals(
    ...
    *,
    ...
    canonical_drift = any(
        ...
        or relative not in canonical_rows
        or _normalized_hash(workspace / relative) != canonical_rows[relative]["content_sha256"]
        for relative in DYNAMIC_CANONICAL_FILES
    )
    if previous.get("signature") == signature and not canonical_drift:
        return [], signature
    ...
    active_document = router_document(
        ...
        sections=[
            ...
            },
            {
                ...
                "content": f"Active task route: {active_ref}",
            },
        ],
    )
