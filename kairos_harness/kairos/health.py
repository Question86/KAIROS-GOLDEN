...
LEGACY_ACTIVE_MARKERS = ("Life Operator", "Keeper-Clean-Loop1")
    ...
    *,
    ...
    for path in documents:
        ...
        try:
            ...
            if ABSOLUTE_FILE_REFERENCE.search(text):
                failures.append(f"outside-workspace file reference in {relative}")
            for marker in LEGACY_ACTIVE_MARKERS:
                if marker.casefold() in text.casefold():
                    failures.append(f"legacy active marker {marker!r} in {relative}")
    ...
    for source_path, prepared in sorted(prepared_by_path.items()):
        for _, reference in prepared.references:
            ...
            if reference.version not in {"dynamic", "live"} and reference.target_id:
    ...
    try:
        ...
        if missing_reference_revisions:
            failures.append(
                f"reference revisions absent from immutable ledger: {missing_reference_revisions}"
            )
        if unpromoted:
        ...
        foreign_workspace_artifacts = sorted(
            ...
            if row["workspace_id"] != config["workspace_id"]
        )
        scope_violations = [
            str(row["artifact_id"])
            for row in connection.execute(
                ...
                WHERE (a.goal_id IS NOT NULL OR a.milestone_id IS NOT NULL)
                  AND (
        ...
        parent_task_violations = [
            ...
            for row in connection.execute(
                ...
                WHERE a.document_type<>'task' AND a.task_id IS NOT NULL
                  AND (
