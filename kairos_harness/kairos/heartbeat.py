...
def _archive_document(workspace: Path, database: KnowledgeDatabase, loop: int, heartbeat: dict[str, Any]) -> tuple[Path, str]:
    ...
    metadata = {
        ...
        "answers": [
            ...
            {"intent": "evidence", "question": "Which receipts prove loop finalization?", "target": "s-validation"},
            {"intent": "experience", "question": "Which reusable experience emerged from the loop?", "target": "s-experience"},
        ],
        "refs": refs,
        "search_contract": [
            {
                "query": f"Which archive validation receipt census proves finalized loop {loop}?",
...
def _finalize_workspace_locked(workspace: Path) -> dict[str, Any]:
    ...
    try:
        archive_path, archive_id = _archive_document(workspace, database, loop, heartbeat)
